...
### 第二步：硬件连接
请参考下表完成 ESP32 与电调的连接：

| 开发板引脚 (ESP32/Arduino) | 电调 (ESC) 引脚 | 说明 |
| :--- | :--- | :--- |
| **定义的 `ESC_PIN`** | 信号线 (通常为白色或橙色) | 传输命令数据 |
| **GND** | 地线 (通常为黑色) | 必须与开发板共地 |
...
> [!TIP]
> **小贴士**：
> * **仅读取参数**：通常无需接入动力电池，仅靠信号线的逻辑电平即可完成。
